...
func dataSourceVaultAccountRead(d *schema.ResourceData, m interface{}) error {
	LogD.Printf("Finding vault account")
	...
	if d.Get("checkout").(bool) {
		result, err := object.CheckoutPassword()
		if err != nil {
			return err
		}
		if !result.Success {
		...
		//LogD.Printf("Checkout result: %+v", result2)

		if v, ok := result.Result["Password"]; ok {
			d.Set("password", v.(string))
		}

		if d.Get("checkin").(bool) {
			if v, ok := result.Result["COID"]; ok {
				result, err := object.CheckinPassword(v.(string))
				if err != nil {
					return err
				}
				if !result.Success {
