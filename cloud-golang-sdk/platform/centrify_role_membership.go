...
func (o *RoleMembership) AddRoleMembers(action string, uID string) (*restapi.StringResponse, error) {
	if o.ID == "" {
		errormsg := fmt.Sprintf("Missing ID for %s", GetVarType(0))
		logger.Errorf(errormsg)
		return nil, fmt.Errorf(errormsg)
...
func (o *RoleMembership) DeleteRoleMembers(action string, userID string) (*restapi.StringResponse, error) {
	if o.ID == "" {
		errormsg := fmt.Sprintf("Missing ID for %s", GetVarType(0))
		logger.Errorf(errormsg)
		return nil, fmt.Errorf(errormsg)
	...
	actionArg[action] = userids
	queryArg["Users"] = actionArg
	logger.Debugf("Users to be deleted: %+v", queryArg)
	resp, err := o.client.CallStringAPI(o.apiUpdate, queryArg)
	if err != nil {
		logger.Errorf(err.Error())
		return nil, err
