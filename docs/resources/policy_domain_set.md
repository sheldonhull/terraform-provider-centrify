...
**domain_set** is a sub attribute in settings attribute within **centrify_policy** Resource.
...
resource "centrify_policy" "test_policy" {
    ...
    policy_assignment = [
        format("VaultDomain|%s", data.centrify_manualset.test_set.id),
    ]
...
More examples can be found [here](https://github.com/centrify/terraform-provider-centrify/blob/main/examples/centrify_policy/policy_domain_set.tf)
